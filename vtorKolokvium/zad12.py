...
if __name__ == '__main__':
    ...
    scaler.fit([row[:-1] for row in dataset])
    ...
    classifier = MLPClassifier(hidden_layer_sizes=hidden_layer, activation='relu',
                               learning_rate_init=learning_rate, max_iter=20, random_state=0)
    ...
    new_sample = scaler.transform([new_sample])[0]

    if accuracy_train - accuracy_test >= accuracy_test * 0.15:
        train_x1 = []
        for row in train_x:
            new_row = [row[i] for i in range(len(row)) if i != col_index]
            train_x1.append(new_row)

        new_sample = [new_sample[i] for i in range(len(new_sample)) if i != col_index]


        classifier1 = MLPClassifier(hidden_layer_sizes=hidden_layer, activation='relu',
                                   learning_rate_init=learning_rate, max_iter=20, random_state=0)

        classifier1.fit(train_x1, train_y)
        new_prediction = classifier1.predict([new_sample])[0]
