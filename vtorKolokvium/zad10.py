...
if __name__ == '__main__':
    ...
    train_X2 = []
    for row in train_X:
        new_row = [row[i] for i in range(len(row)) if i != least_important]
    ...
    train_Xs = scaler1.fit_transform(train_X2)
    test_Xs = scaler1.transform(test_X2)

    train_Xm = scaler2.fit_transform(train_X2)
    test_Xm = scaler2.transform(test_X2)

    classifier2 = MLPClassifier(hidden_layer_sizes=15, random_state=0, activation='relu', learning_rate_init=0.001,
                                max_iter=200)
    classifier2.fit(train_Xs, train_Y)

    classifier3 = MLPClassifier(hidden_layer_sizes=15, random_state=0, activation='relu', learning_rate_init=0.001,
                                max_iter=200)
    classifier3.fit(train_Xm, train_Y)

    predictionsS = classifier2.predict(test_Xs)
    predictionsM = classifier3.predict(test_Xm)

    print(f"Tocnost so StandardScaler: {accuracy_score(test_Y, predictionsS)}")
    print(f"Tocnost so MinMaxScaler: {accuracy_score(test_Y, predictionsM)}")
